param(
    [string]$PluginName = "excel-professional-formatting",
...
function Update-Marketplace {
    param([string]$Path, [string]$PluginName, [string]$SourcePath)
...
function Enable-PluginInConfig {
    param([string]$Path, [string]$PluginName)
...
if ($DryRun) {
    ...
    if ($Register) {
        Write-Plan "Will update marketplace entry and enable excel-professional-formatting in config.toml"
    }
    exit 0
}
