...
PLUGIN_ROOT = Path(__file__).resolve().parent.parent / "core" / "_assets" / "plugins" / "mstack-codex" / "skills"
FORBIDDEN_TERMS = (
    "CLAUDE.md",
    ".claude",
    ".claude-prompts",
...
def test_packaged_plugin_skills_match_skills_codex_tree() -> None:
    assert PLUGIN_ROOT.exists()
    plugin_skill_dirs = sorted(path for path in PLUGIN_ROOT.iterdir() if path.is_dir())
    assert [path.name for path in plugin_skill_dirs] == [path.name for path in _skill_dirs()]

    for skill_dir in _skill_dirs():
        plugin_skill = PLUGIN_ROOT / skill_dir.name
        assert plugin_skill.exists(), plugin_skill
        assert (plugin_skill / "SKILL.md").read_text(encoding="utf-8") == (
            skill_dir / "SKILL.md"
        ).read_text(encoding="utf-8")
        assert (plugin_skill / "agents" / "openai.yaml").read_text(encoding="utf-8") == (
...
def test_codex_skill_boundary_language_is_present() -> None:
    ...
    assert "fixed four-lane decision engine" in texts["pipeline-coordinator"]
...
def test_codex_skill_metadata_reflects_boundaries() -> None:
    ...
    assert "fixed four-lane decision engine" in interfaces["pipeline-coordinator"]["default_prompt"].lower()
...
def test_pipeline_coordinator_references_exist() -> None:
    coordinator_dir = ROOT / "pipeline-coordinator"
    skill_text = (coordinator_dir / "SKILL.md").read_text(encoding="utf-8")
    expected_refs = [
        "coordinator-input-contract.md",
        "embedded-orchestrate-analysis.md",
        "embedded-scenario-scorer.md",
        "embedded-analysis-verifier.md",
        "four-agent-contract.md",
    ]

    for ref_name in expected_refs:
        ref_path = coordinator_dir / "references" / ref_name
