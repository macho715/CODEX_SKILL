...
## Windows Operational Note

- In CLI-backed Windows runs, Node toolchain failures can come from an
  overgrown or heavily duplicated machine `PATH`, not only from missing
  dependencies.
- MStack applies a local mitigation for Node-family subprocesses, but the skill
  should still tell the user to clean up system `PATH` entries when `cmd.exe`
  cannot reliably resolve `node` or `npm`.
