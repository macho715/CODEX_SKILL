...
from .pipeline_recipes import GenericImplementBackend, ImplementRecipeContext, run_implement_recipe
from .types import Preset, RouterResult, StageResult, StageStatus
...
NODE_COMMAND_EXECUTABLES = ("node", "npm", "npx")
...
def _run_implement_stage(context: PipelineRunnerContext) -> StageResult:
    return run_implement_recipe(
        ImplementRecipeContext(
            project_dir=context.project_dir,
            request=context.request,
            lang=context.preset.lang,
        ),
        fallback_backend=context.generic_implement_backend,
    )
...
def _execute_command(project_dir: Path, command: str) -> subprocess.CompletedProcess[str]:
    env = _build_command_env(project_dir, command)
...
def _build_command_env(project_dir: Path, command: str) -> dict[str, str]:
    env = os.environ.copy()
    existing = env.get("PYTHONPATH")
    env["PYTHONPATH"] = (
        f"{project_dir}{os.pathsep}{existing}" if existing else str(project_dir)
    )
    if os.name == "nt" and _is_node_shell_command(command):
        env["PATH"] = os.pathsep.join(_build_windows_node_path_entries(project_dir))
        return env
...
def _is_node_shell_command(command: str) -> bool:
    executable = _extract_executable(command)
    return executable is not None and executable.lower() in NODE_COMMAND_EXECUTABLES


def _build_windows_node_path_entries(project_dir: Path) -> list[str]:
    ...
    add_path(str(project_dir))
    add_path(str(project_dir / "node_modules" / ".bin"))
...
def _select_summary_line(text: str) -> str:
    ...
    for line in reversed(lines):
        normalized = line[2:].strip() if line.startswith("ℹ ") else line
        if normalized.startswith("# pass ") or normalized.startswith("pass "):
            return f"{normalized.split()[-1]} passed"

    for line in reversed(lines):
        normalized = line[2:].strip() if line.startswith("ℹ ") else line
        if normalized.startswith("# fail ") or normalized.startswith("fail "):
            return f"{normalized.split()[-1]} failed"
