...
Usage:
    ...
    mstack install-codex [--target PATH] [--force] [--dry-run]
    mstack install-codex-plugin [--target PATH] [--force] [--dry-run] [--with-marketplace] [--marketplace-path PATH]
    ...
    mstack pipeline <request> [--mode single|subagent|agent_teams] [--generic-implement none|notes] [--json]
...
from core.assets import install_codex_plugin, install_codex_skills
...
def cmd_install_codex_plugin(args: argparse.Namespace) -> int:
    ...
    print(f"[mstack] Plugin target: {result.target_dir.as_posix()}")
    print(
        f"[mstack] Plugin manifest: {(result.target_dir / '.codex-plugin' / 'plugin.json').as_posix()}"
    )
    print(f"[mstack] Plugin name: {result.plugin_name}")
    print(f"[mstack] Installed: {result.skill_count if result.installed else 0}")
    print(f"[mstack] Skipped: {1 if result.skipped else 0}")
    print(f"[mstack] Overwritten: {1 if result.overwritten else 0}")
    if result.marketplace_path:
        print(f"[mstack] Marketplace path: {result.marketplace_path.as_posix()}")
        print(f"[mstack] Marketplace updated: {1 if result.marketplace_updated else 0}")
    return 0
...
def _format_pipeline_cli_json(invocation) -> dict:
    ...
    return {
        "classification": {
            ...
            "requires_parallel_decision": classification.requires_parallel_decision,
            "decision_engine": classification.decision_engine,
            "coordinator_input_ready": classification.coordinator_input_ready,
        ...
        "summary": {
            ...
            "coordinator_verdict": summary.coordinator_verdict,
...
def cmd_pipeline(args: argparse.Namespace) -> int:
    ...
    return 0 if invocation.pipeline_result.final_status != StageStatus.FAILED else 1
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_install_codex_plugin = sub.add_parser(
        "install-codex-plugin",
        help="Install the packaged MStack Codex plugin bundle",
    )
    p_install_codex_plugin.add_argument(
        "--target",
        default=str(Path.cwd() / "plugins" / "mstack-codex"),
        help="Codex plugin target directory",
    )
    p_install_codex_plugin.add_argument(
        "--force",
        action="store_true",
        help="Overwrite an existing managed MStack plugin dir",
    )
    p_install_codex_plugin.add_argument(
        "--dry-run",
        action="store_true",
        help="Show planned plugin install actions without copying files",
    )
    p_install_codex_plugin.add_argument(
        "--with-marketplace",
        action="store_true",
        help="Create or update a local .agents/plugins/marketplace.json entry",
    )
    p_install_codex_plugin.add_argument(
        "--marketplace-path",
        help="Optional marketplace.json output path override",
    )
    p_install_codex_plugin.set_defaults(func=cmd_install_codex_plugin)
