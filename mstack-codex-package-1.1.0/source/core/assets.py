...
GUIDE_NAME = "MSTACK_SKILL_GUIDE.md"
...
def packaged_skill_guide() -> Path:
    """Return the packaged top-level skill guide path."""
    docs_dir = asset_dir(DOC_DIR)
    guide = docs_dir / GUIDE_NAME
    if not guide.exists():
        raise FileNotFoundError(f"Packaged skill guide not found: {guide}")
    return guide
...
def install_codex_skills(
    ...
    *,
    ...
    if not dry_run:
        shutil.copy2(packaged_skill_guide(), target_dir / GUIDE_NAME)
