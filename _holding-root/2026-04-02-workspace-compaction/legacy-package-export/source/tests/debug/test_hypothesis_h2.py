...
def test_mixed_cost_with_none_values():
    ...
    try:
        ...
        cost2 = cost._c(int(ti * .75), int(to * .75), "sonnet")
        total = cost1 + cost2
...
def test_mixed_cost_direct_simulation():
    ...
    try:
        cost = cost._c(int(ti * .25), int(to * .25), "opus") + cost._c(int(ti * .75), int(to * .75), "sonnet")
        print(f"   cost = ${cost}")
    except Exception as e:
    ...
    try:
        ...
        cost_val = cost._c(int(ti * .25), int(to * .25), "opus") + cost._c(int(ti * .75), int(to * .75), "sonnet")
        print(f"   cost = ${cost_val}")
    except TypeError as e:
        print(f"   TypeError 발생: {e}")
        print(f"   → 정확히 에러 메시지와 일치!")
