...
def test_pipeline_cli_respects_mode_override(tmp_path: Path) -> None:
    ...
    assert result.returncode == 0, result.stderr or result.stdout
    data = json.loads(result.stdout)
    assert data["classification"]["execution_mode"] == "single"
    assert data["classification"]["required_execution_mode"] is None
    assert data["summary"]["execution_mode"] == "single"
    assert data["summary"]["required_execution_mode"] == "direct"
...
def test_pipeline_cli_blocks_when_required_executor_missing(tmp_path: Path) -> None:
    repo = _init_python_repo(tmp_path)
    result = _run_cli(
        "pipeline",
        "Create admin dashboard skeleton",
    ...
    assert result.returncode == 1
    data = json.loads(result.stdout)
    assert data["classification"]["required_execution_mode"] == "agent_teams"
    assert data["summary"]["final_status"] == "blocked"
    assert data["summary"]["blocked_reason"] == (
        "agent_teams executor unavailable and single-agent fallback not approved"
    )
    assert data["pipeline_result"]["stages"][0]["status"] == "blocked"


def test_pipeline_cli_allows_explicit_single_agent_fallback(tmp_path: Path) -> None:
    repo = _init_python_repo(tmp_path)
    result = _run_cli(
        "pipeline",
        "Create admin dashboard skeleton",
    ...
    assert data["classification"]["required_execution_mode"] == "agent_teams"
    assert data["summary"]["execution_mode"] == "single"
    assert data["summary"]["required_execution_mode"] == "agent_teams"
    assert data["summary"]["blocked_reason"] == (
...
def test_pipeline_cli_agent_teams_uses_local_parallel_recipe_executor(tmp_path: Path) -> None:
    repo = _init_python_repo(tmp_path)
    result = _run_cli(
        "pipeline",
        "Add CSV import end-to-end",
    ...
    assert data["classification"]["required_execution_mode"] == "agent_teams"
    assert data["summary"]["final_status"] == "passed"
    assert "applied parallel python-csv-feature implementation with 2 workers" in stages["implement"]["output"]


def test_pipeline_cli_external_executor_requires_codex_cli(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    repo = _init_python_repo(tmp_path)
