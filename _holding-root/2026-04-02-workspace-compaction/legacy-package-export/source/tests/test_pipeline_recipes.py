...
from core.pipeline_recipes import (
    ImplementRecipeContext,
    detect_parallel_executor_capabilities,
    run_implement_recipe,
)
from core.types import Lang, RouterDecision, StageResult, StageStatus
...
def test_detect_parallel_executor_capabilities_for_python_csv_feature(tmp_path: Path) -> None:
    repo = _init_python_feature_repo(tmp_path)

    capabilities = detect_parallel_executor_capabilities(
        ImplementRecipeContext(
            project_dir=repo,
            request="Add CSV import end-to-end",
            lang=Lang.PYTHON,
    ...
    assert capabilities == frozenset(
        {RouterDecision.SINGLE, RouterDecision.SUBAGENT, RouterDecision.AGENT_TEAMS}
    )


def test_detect_parallel_executor_capabilities_for_unmatched_request(tmp_path: Path) -> None:
    repo = _init_python_feature_repo(tmp_path)
    ...
    assert capabilities == frozenset({RouterDecision.SINGLE})
