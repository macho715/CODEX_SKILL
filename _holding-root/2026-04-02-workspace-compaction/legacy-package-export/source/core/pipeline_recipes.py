...
from .types import Lang, RouterDecision, StageResult, StageStatus
...
class ImplementRecipe:
    ...
    parallel_plan: Callable[[ImplementRecipeContext], tuple["ParallelImplementTask", ...] | None] | None = None


GenericImplementBackend = Callable[[ImplementRecipeContext], StageResult | None]
ParallelImplementExecutor = Callable[[Path, tuple["ParallelImplementTask", ...], str], StageResult]


@dataclass(frozen=True)
class ParallelImplementWrite:
    """One file write owned by one parallel worker."""
...
@dataclass(frozen=True)
class ParallelImplementTask:
    """Bounded parallel implement task for one worker."""

    worker_name: str
    summary: str
    writes: tuple[ParallelImplementWrite, ...]
...
def run_implement_recipe(
    ...
    *,
    ...
    parallel_executor: ParallelImplementExecutor | None = None,
) -> StageResult:
    ...
    for recipe in _IMPLEMENT_RECIPES:
        if recipe.match(context):
            if (
                ...
                and recipe.parallel_plan is not None
            ):
                plan = recipe.parallel_plan(context)
                if plan:
                    return parallel_executor(context.project_dir, plan, recipe.name)
...
def detect_parallel_executor_capabilities(
    context: ImplementRecipeContext,
) -> frozenset[RouterDecision]:
    """Detect which execution modes can be satisfied by local parallel recipes."""
    capabilities = {RouterDecision.SINGLE}
    for recipe in _IMPLEMENT_RECIPES:
        if not recipe.match(context) or recipe.parallel_plan is None:
            continue
        plan = recipe.parallel_plan(context)
        if not plan:
            continue
        capabilities.add(RouterDecision.SUBAGENT)
        if len(plan) >= 2:
            capabilities.add(RouterDecision.AGENT_TEAMS)
        break
...
def _parallel_plan_python_csv_feature(context: ImplementRecipeContext) -> tuple[ParallelImplementTask, ...]:
    code_dir = _code_dir(context.project_dir)
    import_target = _module_import_path(code_dir, "csv_import")
    return (
        ParallelImplementTask(
            worker_name="csv-module",
            summary="Create csv_import module",
            writes=(
                ParallelImplementWrite(
                    relative_path=_to_posix((code_dir / "csv_import.py").relative_to(context.project_dir)),
                    content=_python_csv_import_module_text(),
                ),
            ),
            owned_paths=("src/csv_import.py",),
        ),
        ParallelImplementTask(
            worker_name="csv-tests",
            summary="Create csv import tests",
            writes=(
                ParallelImplementWrite(
                    relative_path="tests/test_csv_import.py",
                    content=_python_csv_import_test_text(import_target),
                ),
            ),
            owned_paths=("tests/test_csv_import.py",),
        ),
    )
...
def _parallel_plan_python_parser_bugfix(
    context: ImplementRecipeContext,
    ...
    return (
        ParallelImplementTask(
            worker_name="parser-module",
            summary="Patch parser module for empty-input handling",
            writes=(
                ParallelImplementWrite(
                    relative_path=_to_posix(parser_path.relative_to(context.project_dir)),
                    content=_python_parser_bugfix_module_text(),
                    allow_overwrite=True,
                ),
            ),
            owned_paths=(_to_posix(parser_path.relative_to(context.project_dir)),),
        ),
        ParallelImplementTask(
            worker_name="parser-tests",
            summary="Add parser regression tests",
            writes=(
                ParallelImplementWrite(
                    relative_path="tests/test_parser_regression.py",
                    content=_python_parser_bugfix_test_text(import_target),
                ),
            ),
            owned_paths=("tests/test_parser_regression.py",),
        ),
    )
...
def _parallel_plan_python_refactor(
    context: ImplementRecipeContext,
) -> tuple[ParallelImplementTask, ...] | None:
    app_path = _find_existing_module(context.project_dir, "app.py")
    if app_path is None:
    ...
    return (
        ParallelImplementTask(
            worker_name="app-refactor",
            summary="Update app module to delegate to helper",
            writes=(
                ParallelImplementWrite(
                    relative_path=_to_posix(app_path.relative_to(context.project_dir)),
                    content=_python_refactor_app_text(helper_import),
                    allow_overwrite=True,
                ),
            ),
            owned_paths=(_to_posix(app_path.relative_to(context.project_dir)),),
        ),
        ParallelImplementTask(
            worker_name="helper-module",
            summary="Create math helper module",
            writes=(
                ParallelImplementWrite(
                    relative_path=_to_posix((app_path.parent / "math_helpers.py").relative_to(context.project_dir)),
                    content=_python_refactor_helper_text(),
                ),
            ),
            owned_paths=(_to_posix((app_path.parent / "math_helpers.py").relative_to(context.project_dir)),),
        ),
        ParallelImplementTask(
            worker_name="helper-tests",
            summary="Add helper module tests",
            writes=(
                ParallelImplementWrite(
                    relative_path="tests/test_math_helpers.py",
                    content=_python_refactor_test_text(helper_import),
                ),
            ),
            owned_paths=("tests/test_math_helpers.py",),
        ),
    )
...
def _parallel_plan_typescript_csv_feature(context: ImplementRecipeContext) -> tuple[ParallelImplementTask, ...]:
    code_dir = _code_dir(context.project_dir)
    return (
        ParallelImplementTask(
            worker_name="ts-module",
            summary="Create TypeScript csv import module",
            writes=(
                ParallelImplementWrite(
                    relative_path=_to_posix((code_dir / "csvImport.ts").relative_to(context.project_dir)),
                    content=_typescript_csv_import_module_text(),
                ),
            ),
            owned_paths=(_to_posix((code_dir / "csvImport.ts").relative_to(context.project_dir)),),
        ),
        ParallelImplementTask(
            worker_name="ts-tests",
            summary="Create TypeScript csv import tests",
            writes=(
                ParallelImplementWrite(
                    relative_path="tests/csvImport.test.ts",
                    content=_typescript_csv_import_test_text(),
                ),
            ),
            owned_paths=("tests/csvImport.test.ts",),
        ),
    )
