...
description: >
  ...
  설계만 필요하면 mstack-plan, 검증만은 mstack-qa,
  one-shot 처음→끝 워크플로우는 mstack-pipeline 사용.
---
...
- 리뷰/QA 증거가 있고 머지/배포 준비도 판단이 주 작업일 때
- 사용자가 커밋, push, 머지, ship, 릴리스, 핸드오프를 요청할 때
- 브랜치 전략, 커밋 위생, 품질 게이트를 함께 점검해야 할 때

## Prefer Another Skill When

- 설계/코딩 전 계획: `mstack-plan` 사용
- 릴리스 결정 없는 검증만: `mstack-qa` 사용
- one-shot 처음→끝 실행: `mstack-pipeline` 사용
