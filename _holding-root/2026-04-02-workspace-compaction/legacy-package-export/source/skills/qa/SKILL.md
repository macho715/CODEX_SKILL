...
description: >
  ...
  /investigate로 이어진다.
  실패 원인 분석은 mstack-investigate, 리뷰/배포/회고 포함 다단계는 mstack-pipeline,
  구현 계획은 mstack-plan 사용.
---
...
- 코드 변경 후 검증, 테스트 선택, 회귀 커버리지가 주 작업일 때
- 사용자가 diff-aware, full, quick 검증을 요청할 때
- 릴리스 준비도가 설계/근본 원인이 아닌 테스트 결과에 달려 있을 때

## Prefer Another Skill When

- 실패의 근본 원인 분석: `mstack-investigate` 사용
- 리뷰, 배포, 회고 포함 다단계 체인: `mstack-pipeline` 사용
- 구현 계획 미수립: `mstack-plan` 사용
