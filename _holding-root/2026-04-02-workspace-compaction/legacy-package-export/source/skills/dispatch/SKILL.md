...
description: >
  ...
  one-shot end-to-end 실행은 mstack-pipeline, 설계만은 mstack-plan,
  코드 리뷰만은 mstack-review 사용.
---
...
- 직접 실행 vs subagent vs 병렬 팀 중 어떤 방식이 최적인지 결정할 때
- 멀티 파일/크로스 모듈 작업의 소유권 분할, 조율, 검증 라우팅이 필요할 때
- 사용자가 작업 분배, 위임, 병렬화를 요청할 때

## Prefer Another Skill When

- one-shot end-to-end SDLC 실행: `mstack-pipeline` 사용
- 설계/구현 계획만 필요: `mstack-plan` 사용
- 코드/diff 리뷰만 필요: `mstack-review` 사용
