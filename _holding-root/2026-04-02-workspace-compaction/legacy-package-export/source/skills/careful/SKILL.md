...
description: >
  ...
  계획은 mstack-plan, 라우팅은 mstack-dispatch, end-to-end는 mstack-pipeline 사용.
  리뷰/QA/배포/조사는 해당 스킬에 mstack-careful을 안전 레이어로 결합하라.
---
...
- 요청이 데이터 파괴, 시크릿 노출, 히스토리 리라이트, 롤백 비용이 큰 작업일 때
- 공유 모듈, protected branch, 프로덕션 경로, freeze 모드 조사를 다룰 때
- 사용자가 "조심해서", safety, protection, 실수 방지를 요청할 때

## Prefer Another Skill When

- 계획이 주 작업: `mstack-plan` 사용
- 라우팅/소유권이 주 작업: `mstack-dispatch` 사용
- end-to-end 자동 실행: `mstack-pipeline` 사용
- 리뷰, QA, 배포, 조사가 주 작업: 해당 스킬에 `mstack-careful`을 안전 레이어로 결합
